...
(function () {
    'use strict';
    // 全局讲座列表，任何地方均通过 window.lectureList 访问
    window.lectureList = [];
...
3. 将用户名，密码，软件ID分别复制到设置的三个框内`);
    ...
    const vueApp = unsafeWindow.__vueApp__; // 举例
    console.log(vueApp);
    ...
    const lecture_headers = {
        ...
        'Referer': location.origin + '/gsapp/sys/yddjzxxtjappseu/*default/index.do',
    ...
    function insertButtonsInTr(tr) {
        ...
        if (firstTd && fourthTdSpanText) {
            ...
            button.addEventListener('click', async function () {
                ...
                // 确保 lectureList 已经赋值
                const list = await waitForLectureList();
                const lecture = findMatchingObject(fourthTdSpanText, list);
                console.log(lecture)

                if (!lecture) {
                    alert("❌ 未找到匹配的讲座对象，请刷新页面重试");
    ...
    function getTargetLecture() {
        fetch('https://ehall.seu.edu.cn/gsapp/sys/yddjzxxtjappseu/modules/hdyy/queryActivityList.do', {
            method: 'POST',
            ...
            .then(response => {
                if (response.status === 403) {
                    // 处理403错误
                    return fetch(location.origin + "/gsapp/sys/yddjzxxtjappseu/*default/index.do#/hdyy")
                        .then(response => {
...
async function waitForLectureList(maxWaitMs = 10000, intervalMs = 300) {
    return new Promise(resolve => {
        const start = Date.now();

        const pickList = () => {
            // 优先使用脚本内的全局 window.lectureList（我们在 getTargetLecture 中赋值）
            if (Array.isArray(window.lectureList) && window.lectureList.length > 0) {
                return window.lectureList;
            }
            // 其次尝试从页面上下文拿（若有）
            const vue = unsafeWindow?.vm || unsafeWindow?.app || window.app;
            const list = vue?.lectureList || vue?.data?.lectureList || unsafeWindow?.lectureList;
        ...
        const tryResolve = () => {
            ...
            if (Date.now() - start >= maxWaitMs) {
                clearInterval(timer);
                // 超时后也返回当前可见的全局变量，避免一直卡住
                resolve(window.lectureList || []);
            }
        };

        // 立刻尝试一次
        const timer = setInterval(tryResolve, intervalMs);
        tryResolve();
    });
}

function findMatchingObject(searchString, list = window.lectureList) {
    if (!Array.isArray(list)) {
        console.error("❌ lectureList 无效:", list);
        alert("讲座列表未加载，请稍后再试！");
        return null;
    }
    ...
    console.warn("⚠️ 未找到匹配讲座:", searchString);
    return null;
}
    ...
    xhr.open('POST', 'https://upload.chaojiying.net/Upload/Processing.php', false); // false for synchronous request
    ...
    const url = location.origin + "/gsapp/sys/yddjzxxtjappseu/modules/hdyy/getActivityDetail.do";
    ...
    const url = location.origin + '/gsapp/sys/yddjzxxtjappseu/modules/hdyy/addReservation.do';
    ...
    function getLectureVerifyCode(wid) {
        return new Promise((resolve, reject) => {
            const url = location.origin + "/gsapp/sys/yddjzxxtjappseu/modules/hdyy/vcode.do";
...
(function watchVueOrData() {
    let attempts = 0;
    const maxAttempts = 10;
    const tryFindVue = () => {
        // 如果我们已经拿到讲座列表，就不再反复输出日志
        if (Array.isArray(window.lectureList) && window.lectureList.length > 0) {
            return;
        }
        for (const key in unsafeWindow) {
            ...
            if (val && val.$data) {
                console.log("✅ 找到 Vue 实例:", key);
                return;
            }
        }
        if (attempts++ < maxAttempts) {
            setTimeout(tryFindVue, 1000);
        }
    };
    tryFindVue();
})();
